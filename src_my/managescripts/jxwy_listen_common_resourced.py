...
def again_process(pro):
        ...
        read_pid = utils.read_file("common_resourced_pid")
		...
		print " common_resourced 进程不存在，重启进程中..."
                os.chdir(child_path)
                os.system(pro)
        else:
	        print " common_resourced 进程存在"
                        
	time.sleep(1)

def start():
        if os.path.exists(parent_path + "/pids/common_resourced_pid"):
                pid = os.getpid()
                write_file("listen_common_resourced_pid", str(pid))
                while True:
		        again_process("./start_common_resourced.py")
