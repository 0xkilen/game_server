...
namespace nora {
        ...
        class j2pb_error : public exception {
                string _error;
        public:
                j2pb_error(const string &e) : _error(e) {}
                j2pb_error(const FieldDescriptor *field, const string &e) : _error(field->name() + ": " + e) {}
...
#undef _CONVERT
        ...
        static json_t * _pb2json(const Message& msg)
        {
                ...
                for (size_t i = 0; i != fields.size(); i++)
                {
                        ...
                        const string &name = (field->is_extension())?field->full_name():field->name();
...
#define _CONVERT(type, ctype, fmt, sfunc, afunc)                        \
                        case FieldDescriptor::type: {			\
                                ...
                                if (r) throw j2pb_error(field, string("Failed to unpack: ") + error.text); \
        ...
        static void _json2pb(Message& msg, json_t *root)
        {
			...
			//field = d->file()->FindExtensionByName(name);
                        ...
                        if (!field) throw j2pb_error("Unknown field: " + string(name));
        ...
        void json2pb(Message &msg, const char *buf, size_t size)
        {
                ...
                if (!root)
                        throw j2pb_error(string("Load failed: ") + error.text);
