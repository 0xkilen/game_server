...
def again_process(pro):
        ...
        os.chdir(parent_path + "/pids/")
        read_pid = utils.read_file("logind_pid")
...
def start():
        if os.path.exists(parent_path + "/pids/logind_pid"):
                pid = os.getpid()
                write_file("listen_logind_pid", str(pid))
                while True:
		        again_process("./start_logind.py")
