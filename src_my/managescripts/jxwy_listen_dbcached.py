...
def again_process(pro):
        ...
        read_pid = utils.read_file("dbcached_pid")
...
def start():
        if os.path.exists(parent_path + "/pids/dbcached_pid"):
                pid = os.getpid()
                write_file("listen_dbcached_pid", str(pid))
                while True:
		        again_process("./start_dbcached.py")
