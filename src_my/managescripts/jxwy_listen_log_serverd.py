...
def again_process(pro):
        ...
        read_pid = utils.read_file("log_serverd_pid")
		...
		print " log_serverd 进程不存在，重启进程中..."
                os.chdir(child_path)
                os.system(pro)
        else:
	        print " log_serverd 进程存在"
                        
	time.sleep(1)

def start():
        if os.path.exists(parent_path + "/pids/log_serverd_pid"):
                pid = os.getpid()
                write_file("listen_log_serverd_pid", str(pid))
                while True:
		        again_process("./start_log_serverd.py")
