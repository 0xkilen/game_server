...
def again_process(pro):
        ...
        read_pid = utils.read_file("mirrord_pid")
...
def start():
        if os.path.exists(parent_path + "/pids/mirrord_pid"):
                pid = os.getpid()
                write_file("listen_mirrord_pid", str(pid))
                while True:
		        again_process("./start_mirrord.py")
